...
export async function GET(req: NextRequest) {
  try {
    ...
    const countRow: any = await runGet(`SELECT COUNT(*) as cnt FROM bookings ${whereSql}`, params);
    const total = (countRow && countRow.cnt) ? Number(countRow.cnt) : 0;
    const totalPages = Math.max(1, Math.ceil(total / pageSize));

    const offset = (page - 1) * pageSize;
    const rows = await runAll(
      `SELECT id, user_id, screening_id, status, total_amount, number_of_seats, reserved_at, expires_at, created_at FROM bookings ${whereSql} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
